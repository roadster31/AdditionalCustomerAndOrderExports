...
class OrderExport extends AbstractExport
{
    ...
    protected function getQuery(Lang $lang)
    {
        ...
        if (! empty($this->rangeDate)) {
            $query
                ->filterByCreatedAt($this->rangeDate['start']->format('Y-m-d H:i:s'), Criteria::GREATER_EQUAL)
                ->filterByCreatedAt($this->rangeDate['end']->format('Y-m-d H:i:s'), Criteria::LESS_EQUAL);
        }
    ...
    {
        return $this->buildDataSet(Lang::getDefaultLanguage());
    }
}
